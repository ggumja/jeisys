...
export interface EmailSystemSendHistoryRow {
  id: string;
  sent_at: string;
...
const DUMMY_EMAIL_SYSTEM_HISTORY: EmailSystemSendHistoryRow[] = [
  {
    id: 'sys-email-1',
    sent_at: '2026-08-03 09:12',
    customer_name: '하동현',
    email: 'hadh@jeisysmed.com',
    content: '[제이시스] 회원가입 승인 완료 안내',
    status: 'sent',
    fail_reason: '-'
  },
  {
    id: 'sys-email-2',
    sent_at: '2026-08-02 15:45',
    customer_name: '김원장',
    email: 'doctor1@jeisysmed.com',
    content: '[제이시스] 주문 접수 완료 (ORD-178537962)',
    status: 'sent',
    fail_reason: '-'
  },
  {
    id: 'sys-email-3',
    sent_at: '2026-08-01 11:20',
    customer_name: '이원장',
    email: 'doctor2@skinhospital.co.kr',
    content: '[제이시스] 상품 배송 시작 안내 (운송장: 4502819201)',
    status: 'sent',
    fail_reason: '-'
  },
  {
    id: 'sys-email-4',
    sent_at: '2026-07-30 18:05',
    customer_name: '박원장',
    email: 'doctor3@medical.com',
    content: '[제이시스] 비밀번호 재설정 인증 이메일 발송',
    status: 'failed',
    fail_reason: '이메일 반송 (Soft Bounce)'
  }
];
...
export function EmailSystemHistoryPage() {
  const [list, setList] = useState<EmailSystemSendHistoryRow[]>(DUMMY_EMAIL_SYSTEM_HISTORY);
  ...
  const load = async () => {
    ...
    try {
      ...
      const systemItems = data.filter(item => item.send_type === 'system');
      if (systemItems && systemItems.length > 0) {
        const mapped: EmailSystemSendHistoryRow[] = systemItems.map((item, idx) => ({
          id: item.id || `sys-email-${idx}`,
          ...
          email: item.from_email || 'admin@jeisys.com',
          content: item.subject || item.message.slice(0, 25),
          status: item.status === 'sent' ? 'sent' : item.status === 'failed' ? 'failed' : 'checking',
          fail_reason: item.status === 'failed' ? '발송 에러' : '-'
        }));
        setList(mapped);
      } else {
        setList(DUMMY_EMAIL_SYSTEM_HISTORY);
      }
    } catch {
      setList(DUMMY_EMAIL_SYSTEM_HISTORY);
    } finally {
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div className="relative">
            <input
              ...
              placeholder="전송 내용 또는 고객 이름, 이메일 주소로 검색"
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-neutral-50 border-b border-neutral-200 text-xs font-bold text-neutral-700">
              ...
              <th className="py-4 px-4 text-center cursor-pointer select-none hover:bg-neutral-100" onClick={() => handleSort('email')}>
                <div className="flex items-center justify-center gap-1">
                  <ArrowUpDown className="w-3 h-3 text-neutral-400" /> 이메일 주소
                </div>
              </th>
