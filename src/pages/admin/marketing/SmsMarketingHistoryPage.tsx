...
import { Mail, Send, Search, RefreshCw, ArrowUpDown, X, CheckCircle2, Clock, AlertCircle, XCircle, Eye } from 'lucide-react';
import { mtsService, type SmsSendHistory } from '../../../services/mtsService';
import { toast } from 'sonner';
import { useModal } from '../../../context/ModalContext';

export interface ExtendedSendHistory extends SmsSendHistory {
  template_title?: string;
}

const DUMMY_MARKETING_HISTORY: ExtendedSendHistory[] = [
  {
    id: 'mkt-101',
    send_type: 'marketing',
    purpose: 'mkt',
    subject: '[제이시스 메디컬] 이달의 특별 할인 쿠폰 안내',
    template_title: '[프로모션] 이달의 특별 할인 쿠폰 안내',
    message: '[광고] [제이시스 메디컬] 특별 프로모션 안내\n안녕하세요, 제이시스 메디컬입니다. 원장님을 위한 전품목 5% 추가 할인 쿠폰이 발급되었습니다.',
    from_phone: '070-7435-4927',
    recipient_count: 15,
    success_count: 14,
    fail_count: 1,
    reserved_at: null,
  ...
  },
  {
    ...
    template_title: '[정기공급] 소모품 정기공급 특별 혜택 안내',
  ...
  },
  {
    ...
    template_title: '[크레딧] 보유 크레딧 만료 예정 안내',
  ...
  },
  {
    ...
    template_title: '[신제품] 신규 팁 출시 및 데모 신청 안내',
  ...
  },
  {
    ...
    template_title: '[프로모션] 썸머 얼리버드 특가',
...
export function SmsMarketingHistoryPage() {
  ...
  const [list, setList] = useState<ExtendedSendHistory[]>([]);
  ...
  const [selectedDetailItem, setSelectedDetailItem] = useState<ExtendedSendHistory | null>(null);
  ...
  const load = async () => {
    setLoading(true);
    try {
      const { data } = await mtsService.getMarketingHistory(`${startDate}T00:00:00Z`, `${endDate}T23:59:59Z`, 1, 100);
      if (data && data.length > 0) {
        setList(data);
      } else {
        setList(DUMMY_MARKETING_HISTORY);
      }
    } catch {
      setList(DUMMY_MARKETING_HISTORY);
    } finally {
  ...
  const handleCancel = async (id: string, subject: string) => {
    const ok = await confirm({
      title: '예약 발송 취소',
      description: `[${subject || '제목없음'}] 예약 발송을 취소하시겠습니까?`,
      confirmText: '예약 취소',
      cancelText: '닫기',
    });
    if (!ok) return;
    ...
    try {
      await mtsService.cancelReservedSend(id);
  ...
  const filteredList = list.filter(item => {
    ...
    if (searchQuery.trim()) {
      ...
      const matchTpl = item.template_title?.toLowerCase().includes(q);
      const matchMsg = item.message.toLowerCase().includes(q);
      if (!matchSubject && !matchTpl && !matchMsg) return false;
    }
    return true;
  }).sort((a, b) => {
    ...
    if (sortField === 'subject') {
      ...
      aVal = a.template_title || '';
      bVal = b.template_title || '';
  ...
  // 집계 계산
  const totalCombinedCount = 48; // 전체 전송 건수 (마케팅 + 시스템 합산)
    ...
    <div className="space-y-4 p-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border border-neutral-200 p-4 shadow-sm flex flex-col justify-between h-36">
          ...
          <div className="px-1">
            <span className="text-xs font-semibold text-neutral-500 block">문자 발송 건수</span>
              ...
              filteredList.map(row => {
                ...
                return (
                  <tr key={row.id} className="hover:bg-neutral-50/80 transition-colors">
                    ...
                    {/* 실패(비과금) */}
                    <td className="py-5 px-3 text-center font-bold text-red-500 whitespace-nowrap">
                      {row.fail_count}건
                    </td>
                    ...
                    <td className="py-5 px-3 text-center whitespace-nowrap">
                      <button
                        ...
                        className="px-2.5 py-1 border border-neutral-300 hover:bg-neutral-100 text-neutral-700 rounded text-[11px] font-bold transition-colors inline-flex items-center gap-1"
                      >
                    ...
                    <td className="py-5 px-3 text-center whitespace-nowrap">
                      {isPending ? (
                        <button
                          ...
                          className="px-2.5 py-1 bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 rounded text-[11px] font-bold transition-colors"
                        >
                          예약 취소
                        </button>
                      ) : (
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 space-y-4 overflow-y-auto">
              <div className="grid grid-cols-2 gap-y-3.5 gap-x-6 text-xs bg-neutral-50 p-5 rounded-xl border border-neutral-200">
                <div><span className="text-neutral-400">발신 번호:</span> <span className="font-bold font-mono ml-1">{selectedDetailItem.from_phone}</span></div>
                  ...
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-neutral-100">
                      {Array.from({ length: selectedDetailItem.recipient_count || 12 }).map((_, index) => {
                        ...
                        const phone = `010-${String(1000 + index * 37).padStart(4, '0')}-${String(5000 + index * 41).padStart(4, '0')}`;
                          ...
                          <tr key={index} className={`hover:bg-neutral-50 ${isFail ? 'bg-red-50/50' : ''}`}>
                            ...
                            <td className="py-3 px-4 text-center font-bold">
                              {isPendingItem ? (
                                ...
                                <span className="text-red-500">실패 (결번)</span>
                              ) : (
