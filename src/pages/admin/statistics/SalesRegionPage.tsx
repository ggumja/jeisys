...
export function SalesRegionPage() {
  ...
  const handleExport = useCallback(() => {
    ...
    try {
      ...
      const headers = ['순위', '지역', '구매고객 수', '총 주문건수', '평균 주문액', '매출 비중', '누적 매출액'];
      const body = regionData.map((t: any) => [
        t.rank,
        t.region,
        t.customers,
        t.orders,
      ...
      ws['!cols'] = [{ wch: 8 }, { wch: 15 }, { wch: 15 }, { wch: 15 }, { wch: 20 }, { wch: 12 }, { wch: 20 }];

      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, '지역별 매출 순위');

      const now = new Date();
      const dateSuffix = `${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}`;
      XLSX.writeFile(wb, `지역별_매출순위_${dateSuffix}.xlsx`);
    } catch (error) {
      console.error('지역별 매출 순위 엑셀 다운로드 실패:', error);
    }
  }, [regionData, label]);
  ...
  }, [regionData, handleExport, onRegisterExport]);

  useEffect(() => {
    async function fetchStats() {
      setIsLoading(true);
      try {
        const stats = await adminService.getSalesRegionStats(dateRange);
        setRegionData(stats);
      } catch (err) {
        console.error(err);
      } finally {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 border-b border-neutral-200">
          <div>
            <h3 className="font-semibold text-neutral-900 flex items-center gap-2">
              <MapPin className="w-4 h-4 text-[#21358D]" />
              <span>지역별 매출 순위</span>
            </h3>
            <p className="text-xs text-neutral-500 mt-1">배송지 주소를 기반으로 집계된 각 지역별 누적 매출 및 점유율 현황입니다.</p>
          </div>
        </div>
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-neutral-50 border-b border-neutral-200">
              <tr>
                ...
                <th className="py-3.5 px-6 font-semibold text-neutral-700">지역</th>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7} className="text-center py-16 text-neutral-400">
                    조회된 지역별 매출 데이터가 없습니다.
                  </td>
                </tr>
              ) : (
                regionData.map((t) => (
                  <tr key={t.region} className="hover:bg-neutral-50/50 transition-colors">
