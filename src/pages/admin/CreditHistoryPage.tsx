...
export function CreditHistoryPage() {
  ...
  const fetchTransactions = async (page: number, size: number, search: string, type: string, silent = false) => {
    ...
    try {
      const result = await adminService.getAllCreditTransactions(page, size, search, type);
  ...
  const handleExcelDownload = async () => {
    ...
    try {
      // 0은 전체 데이터 조회를 뜻함
      const result = await adminService.getAllCreditTransactions(0, 0, searchTerm, typeFilter);
      const allData = result.data as CreditTransaction[];
      
      const headers = ['일시', '아이디', '회원명', '병원명', '구분', '변동 크레딧(원)', '상세내용', '관련 주문ID'];
