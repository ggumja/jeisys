...
export function PointHistoryPage() {
  ...
  const fetchTransactions = async (page: number, size: number, search: string, type: string, silent = false) => {
    ...
    try {
      ...
      const result = await adminService.getAllPointTransactions(page, size, search, type, startStr, endStr);
  ...
  const handleExcelDownload = async () => {
    ...
    try {
      ...
      XLSX.writeFile(wb, `전체포인트이력_${dateSuffix}.xlsx`);
    } catch (error) {
  ...
  const getTypeBadge = (type: PointTransaction['type']) => {
    switch (type) {
      case 'issue':
        return <Badge variant="outline" className="bg-blue-100 text-blue-800 border-blue-200">지급</Badge>;
      case 'use':
