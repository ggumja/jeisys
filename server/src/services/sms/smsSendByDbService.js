...
module.exports = {
  requestSmsMessageByDb: async function( conn,
    { storeId, sendId, toPhoneNumber, fromPhoneNumber, message }
  ) {
    ...
    let results = await conn.query( sql.insertSmsMessage, [
      storeId, sendId, toPhoneNumber, fromPhoneNumber, message, tranDate ] )
    return results.affectedRows > 0
  },
  requestMmsMessageByDb: async function( conn,
    { storeId, sendId, toPhoneNumber, fromPhoneNumber, subject, message }
  ) {
    let tranDate = moment().format( 'YYYY-MM-DD HH:mm:ss' )
    ...
    let results = await conn.query( sql.insertMmsMesage, [ 
  ...
  requestAlimtalkMessageByDb: async function( conn,
    ...
    let results = await conn.query( sql.insertAlimtalkMessage, [
      storeId, sendId, toPhoneNumber, fromPhoneNumber,
      senderKey, templateCode, message, button,
