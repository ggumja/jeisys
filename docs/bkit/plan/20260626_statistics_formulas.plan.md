...
- **이전 동기 기간 ($PrevStart \sim PrevEnd$) 계산법**:
  - **일별(daily)/주별(weekly)/기타**:
    - 기간 일수 $D =$ 현재 기간 종료일 $-$ 현재 기간 시작일 $+ 1$
    - 이전 동기 시작일 $=$ 현재 기간 시작일 $- D$
    - 이전 동기 종료일 $=$ 현재 기간 시작일 $- 1$일
  - **월별(monthly)**:
    - 기간 개월수 $M = ($종료일의 연도 $-$ 시작일의 연도$) \times 12 + ($종료일의 월 $-$ 시작일의 월$) + 1$
    - 이전 동기 시작일 $=$ 현재 기간 시작일 $- M$개월 (Date 객체 월 기준 조작)
    - 이전 동기 종료일 $=$ 현재 기간 시작일 $- 1$일 (즉, 이전 동기 최종 월의 마지막 날)
  - **연별(yearly)**:
    - 기간 연도수 $Y =$ 현재 기간 종료일 연도 $-$ 현재 기간 시작일 연도 $+ 1$
    - 이전 동기 시작일 $=$ 현재 기간 시작일 $- Y$년
    - 이전 동기 종료일 $=$ 현재 기간 시작일 $- 1$일
...
1. **코드 정밀 비교**: 내부 서비스 코드 내의 모든 쿼리 조건 및 계산 로직이 상기 서술된 계산 공식과 동일한 데이터 속성 및 필터를 참조하는지 확인합니다.
